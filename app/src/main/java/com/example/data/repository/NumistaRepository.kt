...
object NumistaRepository {
     ...
     * ya en `existingTitleKeys`. No sustituye el catálogo estático: solo añade por encima
     * lo que Numista tenga catalogado y aún no esté en la app.
     */
    suspend fun discoverNewCommemorativeCoins(
        ...
        val key = getApiKey()
        if (key.isBlank()) {
            Log.w(TAG, "Sin NUMISTA_API_KEY configurada — no se puede buscar catálogo nuevo")
        ...
        for (country in countries) {
            val countryName = getEnglishCountryName(country.code, country.name)
            for (yr in fromYear..toYear) {
                try {
                    val query = "2 euro commemorative $countryName $yr"
                    ...
                    for (type in types) {
                        ...
                        if (!isIssuerMatch(type.issuer, country.code)) continue

                        val normalizedKey = "${country.code.uppercase()}_${yr}_${
                            title.lowercase().replace(Regex("[^a-z0-9]+"), "")
                        }"
                        if (normalizedKey in existingTitleKeys || normalizedKey in seenInThisRun) continue
                        seenInThisRun.add(normalizedKey)

                        var imageUrl = type.obverse_thumbnail
                        ...
                        found.add(
                            CatalogCoin(
                                id = "numista_${type.id ?: normalizedKey}",
