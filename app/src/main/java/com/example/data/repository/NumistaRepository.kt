...
object NumistaRepository {
    ...
    /**
     * Resuelve una imagen verificada de Numista para cualquier moneda que se muestre en
     * pantalla, sin depender de listas de palabras clave escritas a mano. Filtra los
     * resultados comparando el nombre de emisor que devuelve la propia API (campo
     * documentado `issuer.name`) contra el nombre en inglés del país de la moneda, en vez
     * de adivinar códigos de emisor de Numista (que no son códigos ISO y no están
     * documentados país por país).
     *
     * Usa caché en memoria por moneda para no repetir peticiones innecesarias contra la
     * cuota de la API.
     */
    suspend fun resolveVerifiedImageUrl(coin: CatalogCoin): String? = withContext(Dispatchers.IO) {
        val cacheKey = "v_${coin.countryCode}_${coin.year}_${coin.title}"
        memoryCache[cacheKey]?.let { return@withContext it }
        ...
        try {
            val countryEnglishName = countryCodeToEnglishName(coin.countryCode)
            val cleanTitle = coin.title
                .replace(Regex("\\d{4}"), "")
                .replace(Regex("[()\\-]"), " ")
                .trim()
            val denomText = if (coin.denomination == com.example.data.model.CoinDenomination.EURO_2_COMMEMORATIVE) "2 euro" else "euro"
            val query = "$denomText $countryEnglishName $cleanTitle".trim()

            val response = apiService.searchTypes(apiKey = key, query = query, count = 20)
            val results = response.types ?: emptyList()

            val matched = results.firstOrNull { type ->
                val issuerName = type.issuer?.name?.lowercase() ?: ""
                issuerName.contains(countryEnglishName.lowercase())
            }

            val thumb = matched?.obverse_thumbnail
            if (!thumb.isNullOrBlank()) {
                // Intenta conseguir una imagen de mayor resolución del detalle del tipo
                val detailUrl = matched.id?.let { id ->
                    try {
                        apiService.getTypeDetail(apiKey = key, id = id).obverse?.picture
                    } catch (e: Exception) {
                        null
                    }
                }
                val finalUrl = detailUrl?.takeIf { it.isNotBlank() } ?: thumb
                memoryCache[cacheKey] = finalUrl
                return@withContext finalUrl
            }
        } catch (e: Exception) {
            Log.e(TAG, "Error resolviendo imagen verificada para ${coin.countryCode} ${coin.year} ${coin.title}", e)
        }
        null
    }

    private fun countryCodeToEnglishName(code: String): String = when (code.uppercase()) {
        "ES" -> "Spain"
        "DE" -> "Germany"
        "FR" -> "France"
        "IT" -> "Italy"
